...
class DocumentModel extends Model{
     ...
     * @return array              文档列表
     */
    public function lists($category, $size=10, $order = '`id` DESC', $status = 1, $field = true){
        $map = $this->listMap($category, $status);
        $page = new Page($this->listCount($category), $size);
        $this->setPage($page->get());
        $list =  $this->field($field)->where($map)->order($order)->limit($page->firstRow,$page->listRows)->select();
        foreach ($list as $key=>$value){
            $list[$key]['title'] = $this->subtext($value['title'], 24);
            $list[$key]['description'] = $this->subtext($value['description'], 118);
        }
        return $list;
    }
     ...
     * @return array              文档列表
     */
    public function search($keyword, $size=10, $order = '`id` DESC', $status = 1, $field = true){
        $map = array(
            'status' => 1,
     ...
     * @return array
     */
    public function near($id, $mod=0){
        $map = array(
            'id'          => array($mod?'lt':'gt',$id),
            'pid'         => 0,
            'status'      => 1,
        ...
        /* 返回前一条数据 */
        $data = $this->field(true)->where($map)->order('id DESC')->find();
        if(!empty($data))
        {
            $data['title'] = $this->subtext($data['title'], 24);
        }
        return $data;
    } 
